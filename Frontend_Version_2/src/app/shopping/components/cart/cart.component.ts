...
import { OrderDetail } from 'src/app/shared/models/order-detail';
...
export class CartComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private toastr: ToastrService, private cartService: CartServiceService,
     private tokenStorageService: TokenStorageService, private router: Router, private orderService: OrderService) { }
    ...
    this.CheckoutFormGroup = this.formBuilder.group(
      {
        ...
        address: new FormControl('', [Validators.required , Validators.pattern('(03|05|07|08|09|01[2|6|8|9])+([0-9]{8})')]),
  ...
  onSubmit() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.order = new Order(this.amount, this.CheckoutFormGroup.value.name, this.CheckoutFormGroup.value.address, this.CheckoutFormGroup.value.phoneNumber, 1, user, this.CheckoutFormGroup.value.description );
